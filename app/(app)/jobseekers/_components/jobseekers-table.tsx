...
// ⚡ Bolt Optimization: Extracted table row into a memoized component.
// This prevents O(N) re-renders across the entire table when a single row's selection state changes.
// Rows now only re-render when their specific `isSelected` state or `jobseeker` data changes.
  ...
  return (
    <TableRow className="hover:bg-primary/5">
      <TableCell>
        <Checkbox
          checked={isSelected}
          onCheckedChange={() => onToggleSelect(jobseeker.id)}
        />
      </TableCell>
      <TableCell>
...
export function JobseekersTable({
  ...
  // ⚡ Bolt Optimization: Stabilized callback reference using `useCallback`
  // and functional state updates `(prev) => ...`
  // This ensures the `toggleSelect` function reference remains constant across table renders,
  // which is required for the `React.memo` wrapping `JobseekerRow` to actually work.
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <Button
                      ...
                      aria-current={
                        initialPage === pageNum ? "page" : undefined
                      }
                    >
                      {pageNum}
                    </Button>
