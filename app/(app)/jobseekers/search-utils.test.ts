...
import { sanitizeSearchQuery, escapeLikeWildcards, validateSortColumn } from "./search-utils.ts";
...
describe("validateSortColumn", () => {
  test("should allow valid columns", () => {
    ...
    assert.strictEqual(validateSortColumn("is_ofw"), "is_ofw");
  });
  ...
  test("should return default for invalid columns", () => {
    ...
    assert.strictEqual(validateSortColumn("drop table"), "created_at");
    assert.strictEqual(validateSortColumn("1=1"), "created_at");
  });
