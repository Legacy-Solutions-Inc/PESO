...
/** Returns YYYY-MM-DD for (today - years). Used for age-bounds → date-of-birth filters. */
...
export async function getJobseekers(
  ...
  try {
    ...
    if (filters.englishProficiency && LANGUAGE_PROFICIENCY_KEYS.includes(filters.englishProficiency as (typeof LANGUAGE_PROFICIENCY_KEYS)[number])) {
      query = query.eq(`language->english->>${filters.englishProficiency}`, "true");
    }
    if (filters.filipinoProficiency && LANGUAGE_PROFICIENCY_KEYS.includes(filters.filipinoProficiency as (typeof LANGUAGE_PROFICIENCY_KEYS)[number])) {
    ...
    if (filters.currentlyInSchool !== undefined && filters.currentlyInSchool !== "") {
      query = query.eq("education->>currentlyInSchool", filters.currentlyInSchool === "true" ? "true" : "false");
    }
    ...
    if (filters.workEmploymentStatus) {
      query = query.eq("work_experience->entries->0->>employmentStatus", filters.workEmploymentStatus);
    }
    ...
    if (filters.skillType && SKILL_OTHER_KEYS.includes(filters.skillType as (typeof SKILL_OTHER_KEYS)[number])) {
    ...
    if (filters.referralProgram && REFERRAL_PROGRAM_KEYS.includes(filters.referralProgram.toLowerCase() as (typeof REFERRAL_PROGRAM_KEYS)[number])) {
      const key = filters.referralProgram.toLowerCase();
      query = query.eq(`skills->pesoUseOnly->referralPrograms->>${key}`, "true");
    }

    // Age bounds (from personal_info.dateOfBirth)
    ...
    if (!Number.isNaN(ageMin) && ageMin >= 0) {
      const maxBirthDate = getDateYearsAgo(ageMin);
      query = query.lte("personal_info->>dateOfBirth", maxBirthDate);
    }
    if (!Number.isNaN(ageMax) && ageMax >= 0) {
      const minBirthDate = getDateYearsAgo(ageMax);
      query = query.gte("personal_info->>dateOfBirth", minBirthDate);
    }
...
export async function exportJobseekersCSV(
  ...
  try {
    ...
    while (hasMore) {
        ...
        // Apply filters (same as getJobseekers)
        if (filters.search) {
          const sanitizedSearch = sanitizeSearchQuery(filters.search);
          if (sanitizedSearch) {
            query = query.or(
        ...
        if (filters.email) {
          query = query.ilike("personal_info->>email", `%${escapeLikeWildcards(filters.email)}%`);
        }
        if (filters.employedType) {
          query = query.eq("employment->>employedType", filters.employedType);
        }
        ...
        const expLangKeys = ["read", "write", "speak", "understand"] as const;
        if (filters.englishProficiency && expLangKeys.includes(filters.englishProficiency as (typeof expLangKeys)[number])) {
          query = query.eq(`language->english->>${filters.englishProficiency}`, "true");
        }
        if (filters.filipinoProficiency && expLangKeys.includes(filters.filipinoProficiency as (typeof expLangKeys)[number])) {
          query = query.eq(`language->filipino->>${filters.filipinoProficiency}`, "true");
        }
        if (filters.currentlyInSchool !== undefined && filters.currentlyInSchool !== "") {
          query = query.eq("education->>currentlyInSchool", filters.currentlyInSchool === "true" ? "true" : "false");
        ...
        if (filters.skillType && expSkillKeys.includes(filters.skillType as (typeof expSkillKeys)[number])) {
          query = query.eq(`skills->otherSkills->>${filters.skillType}`, "true");
        }
        const expReferralKeys = ["spes", "gip", "tupad", "jobstart", "dileep", "tesda_training"] as const;
        if (filters.referralProgram && expReferralKeys.includes(filters.referralProgram.toLowerCase() as (typeof expReferralKeys)[number])) {
          const key = filters.referralProgram.toLowerCase();
          query = query.eq(`skills->pesoUseOnly->referralPrograms->>${key}`, "true");
        }

        // Age bounds (from personal_info.dateOfBirth)
        const exportAgeMin = parseInt(String(filters.ageMin ?? ""), 10);
        const exportAgeMax = parseInt(String(filters.ageMax ?? ""), 10);
        if (!Number.isNaN(exportAgeMin) && exportAgeMin >= 0) {
          const maxBirthDate = getDateYearsAgo(exportAgeMin);
          query = query.lte("personal_info->>dateOfBirth", maxBirthDate);
        }
        if (!Number.isNaN(exportAgeMax) && exportAgeMax >= 0) {
          const minBirthDate = getDateYearsAgo(exportAgeMax);
          query = query.gte("personal_info->>dateOfBirth", minBirthDate);
        }
